...
coroutine.wrap(function()
    ...
    local function check()
        ...
        repeat wait() until game:GetService("Workspace") ~= nil and game:GetService("Workspace"):FindFirstChild("WORKSPACE_Entities") and game:GetService("Workspace")["WORKSPACE_Entities"]:FindFirstChild("Animals") and game:GetService("Workspace")["WORKSPACE_Entities"].Animals:FindFirstChildOfClass("Model")
        wait()
        local animals = {}
        local exists = false
        local c = 0
        ...
        return exists, animals
    end
    local exists, animals = check()
    if exists then
        ...
        for i,v in pairs(animals) do
            rconsoleprint("Found Legendary "..v.Name.." !\n")
            ESP:add(v, "Legendary "..v.Name)
        end
    else
        rconsoleprint("@@LIGHT_RED@@")
